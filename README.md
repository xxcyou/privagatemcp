...
- **简捷模式**:设置页一键开启,关闭动画/毛玻璃/渐变等装饰,首页变成朴素信息列表(地址/Token/JSON 复制、权限/工具/ADB 状态),不花眼
...
- **危险命令策略**(设置页可切):
  - `严格`(默认):拦截 `rm` 关键分区根目录、`dd` 写块设备、`mkfs/fdisk/format` 格式化、fork 炸弹
  - `警告`:放行但审计标记 + 结果提示(刷机/分区等场景)
  - `关闭`:全部放行(风险自负);`rm -rf /` 根目录始终拦截
  - `rm -rf /data/具体路径` 等正常操作不受限(清病毒场景)
- **审计日志**:每条 root/shell 执行落盘 `audit.log`(时间/命令/退出码/耗时),可用 `audit_log` 工具查询
- 输出自动截断 128KB(超大日志如 logcat 不会堵死 MCP 通道)
